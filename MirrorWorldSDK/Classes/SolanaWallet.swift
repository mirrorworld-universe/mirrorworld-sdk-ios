//
//  MirrorWalletMoudle.swift
//  MirrorWorldSDK
//
//  Created by ZMG on 2022/11/1.
//

import UIKit

@objc public class SolanaWallet: MirrorBaseMoudle {
  
    var config:MirrorWorldSDKConfig?
    
    @objc public func getTransactions(limit:Int,next_before:String, onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken { succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "transactions?limit=\(limit)&next_before=\(next_before)")
            MirrorWorldNetWork().request(url: url,method: "Get",params: nil) {[weak self] response in
                self?.handleResponse(response: response) { data in
                    onSuccess?(data)
                } failed: { code, message in
                    onFailed?()
                }
            } _: { code,err in
                DispatchQueue.main.async {
                    onFailed?()
                }
            }
        }
    }

    @objc public func getTransactionsByWallet(wallet_address:String, limit:Int,next_before:String, onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken { succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "\(wallet_address)/transactions?limit=\(limit)&next_before=\(next_before)")
            MirrorWorldNetWork().request(url: url,method: "Get",params: nil) {[weak self] response in
                self?.handleResponse(response: response) { data in
                    onSuccess?(data)
                } failed: { code, message in
                    onFailed?()
                }
            } _: { code, err in
                DispatchQueue.main.async {
                    onFailed?()
                }
            }
        }
    }
    
    @objc public func getTransactionBySignature(signature:String, onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken { succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "transactions/\(signature)")
            MirrorWorldNetWork().request(url: url,method: "Get",params: nil) {[weak self] response in
                self?.handleResponse(response: response) { data in
                    onSuccess?(data)
                } failed: { code, message in
                    onFailed?()
                }
            } _: { code, err in
                DispatchQueue.main.async {
                    onFailed?()
                }
            }
        }
    }
    
    @objc public func getTokens(onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken { succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "tokens")
            MirrorWorldNetWork().request(url: url,method: "Get",params: nil) {[weak self] response in
                self?.handleResponse(response: response) { data in
                    onSuccess?(data)
                } failed: { code, message in
                    onFailed?()
                }

            } _: { code,error in
                DispatchQueue.main.async {
                    onFailed?()
                }
            }
        }
    }
    
    @objc public func getTokensByWallet(wallet_address:String,onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken { succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "tokens/\(wallet_address)")
            MirrorWorldNetWork().request(url: url,method: "Get",params: nil) {[weak self] response in
                self?.handleResponse(response: response) { data in
                    onSuccess?(data)
                } failed: { code, message in
                    onFailed?()
                }

            } _: { code,error in
                DispatchQueue.main.async {
                    onFailed?()
                }
            }
        }
    }

    @objc public func transferSOL(to_publickey:String,amount:Int,onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken {[weak self] succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "transfer-sol")
            let params = [
                "to_publickey":to_publickey,
                "amount":amount
            ]as [String : Any]
            self?.authorization.requestActionAuthorization(config: self?.config,params, actionType:MWActionType.transferSOL,{ success,authToken,errorDesc in
                if success{
                    MirrorWorldNetWork().request(url: url,method:"Post",params:params,authToken) {[weak self] response in
                        self?.handleResponse(response: response) { res in
                            onSuccess?(res)
                        } failed: { code, message in
                            onFailed?()
                        }
                    } _: { code,error in
                        DispatchQueue.main.async {
                            onFailed?()
                        }
                    }
                }
            })
        }
    }


    /**
     *
     *
     */
    @objc public func transferToken(to_publickey:String,amount:Int,token_mint:String,decimals:Int,onSuccess:((_ data:String?)->())?,onFailed:(()->())?){
        self.checkAccessToken {[weak self] succ in
            let url = MirrorUrlUtils.shard.getMirrorUrl(serviceEnum: MirrorService.Wallet, APIPath: "transfer-token")
            let params = [
                "to_publickey":to_publickey,
                "amount":amount,
                "token_mint":to_publickey,
                "decimals":decimals
            ]as [String : Any]
            self?.authorization.requestActionAuthorization(config: self?.config, params,actionType: MWActionType.transferSPLToken, { success, authToken,errorDesc in
                if success{
                    MirrorWorldNetWork().request(url: url,method: "Post",params: params,authToken) {[weak self] response in
                        self?.handleResponse(response: response) { res in
                            onSuccess?(res)
                        } failed: { code, message in
                            onFailed?()
                        }
                    } _: { code,error in
                        DispatchQueue.main.async {
                            onFailed?()
                        }
                    }
                }else{
                    DispatchQueue.main.async {
                        onFailed?()
                    }
                }
            })

        }


    }
    
}
